...
__all__ = ['InputLSTM', 'InputEmbedding', 'MetaNet', 'HiddenMixNN', 'ModelMSMSTransformer', 'ModelMSMSpDeep3',
           'IntenAwareLoss', 'pDeepModel', 'mod_feature_size', 'max_instrument_num', 'frag_types', 'max_frag_charge',
           'num_ion_types', 'nce_factor', 'charge_factor', 'pearson', 'spectral_angle', 'spearman', 'evaluate_msms',
           'add_cutoff_metric']
...
class InputLSTM(torch.nn.Module):
    def __init__(self,
        mod_feature_size,
        out_features,
    ):
...
class InputEmbedding(torch.nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.aa_embedding = model_base.aa_embedding(
...
class MetaNet(torch.nn.Module):
    def __init__(self,
        ...
        self.nn = torch.nn.Linear(
            max_instrument_num+2, out_features
        )
    ...
    def forward(self,
        ...
        inst_x = torch.nn.functional.one_hot(
            instrument_indices, self.max_instrument_num
        )
        meta_x = torch.cat((inst_x, NCEs, charges), 1)
        return self.nn(meta_x)

class HiddenMixNN(torch.nn.Module):
    def __init__(self, hidden, dropout=0.2):
        ...
        self.transormer = model_base.SeqTransformer(
            hidden, hidden, nhead=8, nlayers=1, dropout=dropout
        )
    def forward(self, x):
        x = self.lstm(x)
...
# Cell
class ModelMSMSTransformer(torch.nn.Module):
    def __init__(self,
        mod_feature_size,
        num_ion_types,
        max_instrument_num,
        ...
        self.meta_nn = MetaNet(
            max_instrument_num, hidden
        )

        self.dropout = torch.nn.Dropout(dropout)

        self.input = InputLSTM(
            mod_feature_size, hidden
        )

        self.hidden = HiddenMixNN(hidden, dropout)
...
class pDeepModel(model_base.ModelImplBase):
    def __init__(self,
        dropout=0.2,
        lr=0.001,
        model_class:torch.nn.Module=ModelMSMSTransformer,
    ):
        super().__init__()
