...
__all__ = ['convert_one_pFind_mod', 'translate_pFind_mod', 'get_pFind_mods', 'get_pFind_sites',
           'remove_pFind_decoy_protein', 'pFindReader', 'PSMLabelReader', 'load_psmlabel_list']
...
def get_pFind_mods(pfind_mod_str):
    ...
    else: return ';'.join([item.split(',',1)[1] for item in pfind_mod_str.split(';')])

def get_pFind_sites(pfind_mod_str):
    pfind_mod_str = pfind_mod_str.strip(';')
    if not pfind_mod_str: return ""
    else: return ';'.join([item.split(',',1)[0] for item in pfind_mod_str.split(';')])
...
class pFindReader(PSMReaderBase):
    ...
    def _from_pfind_df(self, pfind_df):
        ...
        psm_df['mods'] = pfind_df['Modification'].apply(get_pFind_mods)
        psm_df['mod_sites'] = pfind_df['Modification'].apply(get_pFind_sites)
...
class PSMLabelReader(pFindReader):
    ...
    def _from_psmlabel_df(self, psmlabel_df):
        ...
        psm_df['mods'] = psmlabel_df['modinfo'].apply(get_pFind_mods)
        psm_df['mod_sites'] = psmlabel_df['modinfo'].apply(get_pFind_sites)
