...
__all__ = ['match_one_raw', 'get_ms2_features', 'match_one_raw_mp', 'get_ms2_features_mp', 'ScoreFeatureExtractor',
           'mp_init', 'ScoreFeatureExtractorMP', 'AD_THREAD_NUM']
...
class ScoreFeatureExtractor:
    ...
    def fine_tune_models(self,
        ...
        logging.log(logging.INFO, 'Preparing for fine-tuning ...')
        psm_df_list = []
        ...
        logging.log(logging.INFO, 'Fine-tuning ...')
        self._tune(
            *concat_precursor_fragment_dataframes(
                psm_df_list,
                matched_intensity_df_list
    ...
    def extract_rt_features(self, psm_df):
        if 'rt_norm' in psm_df.columns:
            psm_df = self.model_mgr.rt_model.predict(
                psm_df
            )
    ...
    def extract_mobility_features(self, psm_df):
        if (
            ...
            psm_df = self.model_mgr.ccs_model.predict(
                psm_df
            )
    ...
    def extract_features(self,
        ...
        if self.require_model_tuning:
            logging.log(logging.INFO, 'Fine-tuning models ...')
        ...
        logging.log(logging.INFO, 'Extracting alphadeep features ...')
        result_psm_list = []
        for raw_name, df in df_groupby_raw:
            ...
            if 'nce' not in df.columns:
                df['nce'] = self.model_mgr.nce
                df['instrument'] = self.model_mgr.instrument
            predict_inten_df = self.model_mgr.ms2_model.predict(df)
        ...
        logging.log(logging.INFO, 'Finish extracting features.')
        return self.psm_df
...
class ScoreFeatureExtractorMP(ScoreFeatureExtractor):
    def __init__(self,
        ...
        # lock idea from https://stackoverflow.com/questions/25557686/python-sharing-a-lock-between-processes
        self.mp_pool = mp.Pool(
            processes=AD_THREAD_NUM, initializer=mp_init, initargs=(_lock,)
        )
    ...
    def fine_tune_models(self,
        ...
        res_list = self.mp_pool.imap_unordered(
            match_one_raw_mp,
        ...
        psm_df_list = []
        matched_intensity_df_list = []
        for df, _, inten_df, _ in res_list:
            psm_df_list.append(df)
            matched_intensity_df_list.append(inten_df)
    ...
    def extract_features(self,
        ...
        result_psm_list = self.mp_pool.imap_unordered(
            self.extract_features_one_raw,
            one_raw_param_generator(df_groupby_raw)
        )

        logging.log(logging.INFO, 'Finish feature extraction multiprocessing ...')
