...
class Model_MS2_CustomTransformer(torch.nn.Module):
    """
    Customize a TransformerEncoderLayer for MS2 prediction using the BERT Encoder architecture, and invoke it through a TransformerEncoder
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if num_modloss_types > 0:
            ...
            self.modloss_nn = torch.nn.ModuleList(
                [
                    building_block.Hidden_PyTorch_TransformerEncoder(
                        hidden,
                        nlayers=1,
                        dropout=dropout,
                        output_attentions=output_attentions,
