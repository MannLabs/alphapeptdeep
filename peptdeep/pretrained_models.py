...
__all__ = ['is_model_zip', 'download_models', 'install_models', 'pretrain_dir', 'model_name', 'model_url',
...
def download_models(
    ...
    if not os.path.isfile(url):
        downloaded_zip = os.path.join(
            pretrain_dir,f'{url_zip_name}.zip'
        )
...
def install_models(downloaded_zip:str, overwrite=True):
    ...
    try:
        ...
        shutil.move(
            os.path.join(pretrain_dir, f'{url_zip_name}/{model_name}'),
            os.path.join(pretrain_dir, model_name)
        )
        os.rmdir(os.path.join(pretrain_dir, url_zip_name))
    except KeyError:
        tar = TarFile(downloaded_zip)
        with open(os.path.join(pretrain_dir, model_name), 'wb') as f:
            f.write(tar.extractfile(
                f'{url_zip_name}/{model_name}'
            ).read())
        tar.close()
    _zip.close()
    logging.info(f'The pretrained models had been installed in {model_zip}')
